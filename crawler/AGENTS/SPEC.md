...
- ✅ Article candidates are refused when the URL path names a listing — category, tag, archive, author/profile, pagination segments (`LISTING_PATH_SEGMENTS` in `fetch.url_matches`, whole segments only). A listing page shows fresh teasers and today's date, so it passes the «published today» gate and gets saved and even published as a news item unless the URL shape stops it (2026-08-11: category, tag and profile pages 9147–9151, 8298, 8690 went out as retold «news»).
...
- ✅ The queue order is the crawler's to decide and the operator's to override: `exchange_publication_plan` holds what a human changed (moved up or down, held until, taken off) and the `exchange_publication_order` view combines it with «сила» computed from the latest scores. News about Russia (pride_russia ≥ 5) gets +1 to its strength, capped at 10, and the view returns `pride_russia` so the publisher can keep its daily quota of such news; «Эфир» replays that quota from the settings the publisher recorded (2026-09-24, migration 0019). The publisher reads that view; the screen shows the same order. A missing view or empty plan means the old behaviour, preparation order.
...
- ✅ A «Картина дня» page: the operator edits each slot of the daily generated picture — enabled flag, caption, local generation time, the prompt pair for the model, the style list (one per line; the pipeline picks one at random, never repeating within the slot's current month), model hints (provider, model, reasoning effort and a «Поиск в интернете» checkbox for the chat call) and the two image sizes (vertical for telegram, horizontal for the sites and VK) — and can add a new slot (the future «картина вечера») by key. The settings live in the crawler DB (`exchange_daypic_slot`, owned by migrations, seeded with the `day` slot switched off) because the pipeline executes them and the rule must be one for both, like the selection thresholds. The same page shows the gallery of past pictures with per-platform links and errors, read read-only from the pipeline DB (`daypic_item`/`daypic_publication`); the picture bytes are served by a login-checked view from the pipeline's daypic directory. «Прогнать сейчас» drops a `run-daypic` request into the shared mailbox — the pipeline makes today's issue immediately, without waiting for the slot's hour. The model-hint fields carry placeholders naming the pipeline's real defaults, and the size fields come pre-filled.
