...
Столбцы: `news_id`, `strength`, `operator_rank`, `hold_until`, `dropped_at`, `pride_russia`.

`strength` - сила новости от 0 до 10: половина её это лучшая из семи сильных сторон, остальное позитивность и интересность. Новость про Россию (`pride_russia` не ниже 5) получает к силе +1, но не больше 10: без надбавки такие новости стояли в очереди ниже всех и протухали (миграция `0019`, 2026-09-24). Время подготовки для порядка худший из признаков: оно говорит, когда до новости дошли руки у машины, а не насколько новость хороша.
...
`pride_russia` - оценка по оси «Гордость за Россию», 0 если её нет. По ней клиент держит суточную квоту новостей про Россию. Вьюха до миграции `0019` этого столбца не имеет, и клиент обязан это пережить: порядок работает, квота просто молчит.
