...
- `styles` — список стилей, по одному на строку; клиент берёт стиль по числу месяца (31-е число упирается в последний пункт списка).
- `chat_provider`/`chat_model`, `image_provider`/`image_model`, `image_size` — подсказки роутеру; пустое значение оставляет выбор клиенту или роутеру.
