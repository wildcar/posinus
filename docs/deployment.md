...
Вместе с сервисом скрипт гасит и то, что его запускает: таймеры и `.path`-юниты из `TriggeredBy`. Иначе таймер конвейера успевал выстрелить прямо посреди обновления, открывал базу — и всё откатывалось (так было 2026-08-09, каждое второе обновление). После обновления и триггеры, и сервисы поднимаются обратно, ровно те, что работали до него.
