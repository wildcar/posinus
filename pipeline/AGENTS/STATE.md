...
- **Repeat check in the preparer, LIVE at `0f7b9d0` since 2026-09-24 (owner-approved deploy).** After the retelling, before pictures: top-8
  ...
  Items already queued are not rechecked: 17706, 19353, 17772, 19455 were dropped from the
  queue by hand (plan note «Повтор новости N», owner-approved). 400 tests.
- **Daily quota of news about Russia, LIVE at `dff5b05` since 2026-09-24 16:02 UTC** (owner
  ...
  view). 393 tests.
- **Final check runs in `shadow` mode on prod since 2026-09-21; decision-model questions `v2`
  since 2026-09-24.** The chat model (prod: `openrouter` / `z-ai/glm-5.3-flash`, pinned by the
  env) still decides; `~typesafe/jev-latest` (served `typesafe/jev-1.13-20260917`) answers
  beside it, both land in `final_check_shadow`. The first 66 `v1` rows (09-21..09-24) agreed
  64/66; chat vetoed nothing, both disagreements were jev false vetoes on `advertising` 0.52.
  ...
  (counts only `v2` rows). Switch the env to `decide` if jev misses no chat veto and adds at
  most 1–2 false ones; otherwise consider «veto if either says no». Base rate: ~1.8% of
  selected items get a chat veto (14 of 776 in 30 days).
...
- **First article in «Интересное» (2026-09-11 ~12:15 UTC, live):** the owner's folder
  ...
  folder-per-article scheme. **Since ~12:45 UTC articles are in the home feed** (card «Статья»)
  and **`posinus-wildcar-org-watch.timer` rebuilds the site by itself** when the hand-edited
  sources (docs/ minus news/kartina, hooks/, overrides/, mkdocs.yml) are newer than the last
  build and quiet for 90 s — the owner drops a folder, the site follows within minutes.
...
- **The channel was silent for 38 hours to 2026-08-16 (last post 15 August 09:15 MSK) because
  ...
  Since 26 July the channel published 178 news against 100 selected, living on a reserve that
  ran out on 14 August, and the profile raised on 11 August cut selection from 4.51% of the
  corpus to 1.60%. **The profile is at revision 4 since 2026-08-16 (`highlight_min` 8 on all
  seven axes, owner's call)**; a full `--backfill --rescore-all` corrected 174 verdicts, of
  which 164 news are already tombstoned, so 8 fresh items (14–15 August) reached the queue.
  Next slot 09:00 MSK.
- **`PREPARER_MAX_TOKENS=4000` is too small for the current retelling prompt**: four of five
  items failed on 2026-08-16 with «DeepSeek stopped at max_tokens=4000 before returning any
  content, produced 4000 completion tokens». Raise it to 8000 in `/etc/posinus/pipeline.env`
  (the agent permission policy refuses to edit that file, so the owner does it). Items left at
  status `error` are retried by the next preparer run on their own.
- News 12306 (the Dolly Parton obituary) is off wildcar.org since 2026-08-26 19:03 UTC,
  by the 8949 procedure: `publication` row flipped to `status='removed'` (reason recorded),
  page dir deleted, index and RSS rebuilt from 228 ok items by the publisher's own
  builders, site rebuilt via the marker. Verified live: page 404, no trace in index or
  feed. The public 404 lagged a few minutes behind the origin — hel-vps's nginx cache
  serves wildcar.org and revalidates on its own TTL; no purge access from this host, and
  none was needed. Telegram (t.me/posinus/751), VK (wall-233237778_301) and wildcar.ru
  copies stay up — the owner decides separately.
- News 8949 (Shotam, military FPV drones for the US army) is off wildcar.org since
  2026-08-11: page dir removed, index and RSS regenerated by the publisher's own builders,
  its `publication` row for `wildcar_org` flipped to `status='removed'` (rows are never
  deleted; SPEC «Идемпотентность и метки»). Telegram/VK/wildcar.ru posts of that news are
  still up — the owner decides. The source ban lives on the crawler side.
...
- 2026-08-09: illustrations are raster-only (LIVE at `1e53d03`, shipped by
  `git pull` in /opt/posinus between timer runs). News 8690/8710 (moya-planeta.ru)
  were prepared with nothing but the site's SVG logos: the download filter took any
  `image/*` (so `image/svg+xml` passed), unknown types were saved as `.img`, and the
  vision check skips extensionless files — so the exact junk it hunts sailed through,
  and no picture was generated because the items were not pictureless. Telegram
  (IMAGE_PROCESS_FAILED), Эгея (upload ignored, status 200) and VK (empty `photo`)
  all rejected the upload; wildcar_org, a file copy, published the logos. Fix:
  downloads keep only `CONTENT_TYPE_EXT` types, a generated picture with unknown
  magic bytes is dropped, 281 tests pass. Repair, all verified live: the three logo
  URLs blacklisted (`--ignore-image`, 5 queue copies purged), `--review-images`
  generated a picture for each emptied item, attempts reset — both went out to all
  platforms (t.me/posinus/627-628, wall-233237778_177-178), and the wildcar.org
  pages were rebuilt with the generated JPG by re-queueing just the wildcar_org rows
  (stale `.img` files removed from the content dir; live pages verified, `.img` 404).
