...
- Next: owner revives the router's codex login (fresh `codex login` under the modelrouter user, or copy the newest bundle from `~/.hermes/auth.json`); the feature arms itself the moment the credential works. Then decide whether generated pictures should carry a visible «изображение сгенерировано» label on the platforms.
