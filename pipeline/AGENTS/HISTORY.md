...
## 2026-09-24 · Финальный контроль через jev: вопросы v2 и отдельный порог рекламы
- What: четыре флага модели решений переписаны (версия `v2`): некролог включает статью о наследии умершего, беда без развязки — опасную болезнь конкретного героя, агитация — политическую колонку и сборник с одним таким сюжетом, реклама — только текст ради продажи. Порог рекламы свой, `EVALUATOR_DECIDE_ADVERTISING_THRESHOLD` (0.6). В `final_check_shadow` колонка `questions`, `--shadow-report` считает только текущую версию. 402 теста.
- Why: за 3 дня тени чат не забраковал ничего, проверить полноту было не на чем. Прогон 14 прошлых браков (тексты скачаны заново) и 106 одобренных: v1 поймал 9/14 при 5 ложных, v2 — 11/14 при 1 ложном. Все ложные браки v1 давала реклама.
- Files: pipeline/evaluator.py, tests/test_evaluator.py, deploy/pipeline.env.example, docs/services.md, AGENTS/SPEC.md
- Next: ~1 октября `--shadow-report` по строкам v2; если jev не пропустит ни одного чат-брака — перевести env в `decide`.

## 2026-09-24 · Проверка на повтор в препараторе
- What: после пересказа новость сравнивается с вышедшими и стоящими в очереди за 30 дней (отсев по основам слов заголовка и тегов, решение за glm-5.3-flash); повтор получает статус `duplicate` и `duplicate_of`, картинки для него не качаются. Краулерский «Эфир» показывает повторы. 400 тестов.
- Why: перепечатки на других языках simhash не ловит — деревня Бейла вышла трижды, всего около 15 повторённых историй за август–сентябрь. Живая проверка: 15 из 15 повторов найдены, 0 ложных на 10 похожих.
- Files: pipeline/preparer.py, tests/test_preparer.py, deploy/pipeline.env.example, docs/services.md, AGENTS/SPEC.md; crawler/collector/services/broadcast.py, stages.py, templates/collector/broadcast.html
- Next: владелец снимает с очереди 17706, 19353, 17772, 19455; через неделю просмотреть список «Повторы».

## 2026-09-24 · Суточная квота новостей про Россию
- What: `PUB_RUSSIA_PER_DAY`=1 / `PUB_RUSSIA_MIN`=5: пока сегодня не вышла новость с pride_russia ≥ 5, лучшая такая из очереди идёт первой после поднятых оператором; `load_plan` читает `pride_russia` из вьюхи и переживает старую. Парная миграция краулера `0019` (+1 к силе) — в его HISTORY. 393 теста.
- Why: с 20 августа ни одна новость про Россию не вышла — сила ~7,2 при нижней границе опубликованного 7,8, все протухли в очереди.
- Files: pipeline/publisher.py, tests/test_publisher.py, deploy/pipeline.env.example, AGENTS/SPEC.md; docs/contracts/database-contract.md
- Next: владелец запускает `update-ubuntu.sh` (первый прогон агента откатился); проверить первый выход по квоте.
...
- Files: wildcar-site: hooks/feed.py, README.md (коммит после fc39898)
- Next: — (папка статьи закоммичена по просьбе владельца, wildcar-site c906fe7).

## 2026-09-11 · Уведомитель: площадка, принявшая пост после последнего отказа, не авария
- What: в обеих проверках «не принимает посты» и «не принимает картину дня» добавлено условие `HAVING`: тревога только если новейшая запись `error` площадки моложе её новейшей `ok`. Три теста.
- Why: в 07:09 МСК владелец получил «ВКонтакте не принимает посты» через 19 часов после последнего отказа и 11 принятых постов: брошенные строки мёртвого токена Kate Mobile ещё лежали в суточном окне, а повтор раз в 12 часов поднял их снова. Ничего лишнего в системе не работало, проверено по журналу, процессам и cron.
- Files: pipeline/notify.py, pipeline/tests/test_notify.py, pipeline/docs/services.md, pipeline/AGENTS/SPEC.md
- Next: —

## 2026-09-10 · wildcar.org: карточки и постраничный архив в разделах, без вводных фраз
- What: индексы разделов на сайте теперь карточки по 20 на страницу, страницы 2…N хук `hooks/feed.py` создаёт виртуально (`/news/page/2/`, `on_files`); из генерируемых индексов убраны фразы «Хорошие новости, которые отобрала и пересказала машина posinus» и «Каждый день машина рисует картину…» (остались H1 и у новостей ссылка на RSS). На главной снят баннер-обложка; в подвале логотипы Дзена и сайта снова квадратные (фиксированный бокс `.md-social__link` и `max-width:100%` сжимали PNG). 350 тестов.
- Why: просьба владельца после просмотра первой версии.
- Files: pipeline/publisher.py, pipeline/daypic.py; wildcar-site: hooks/feed.py, docs/index.md, docs/assets/stylesheets/posinus.css, README.md
- Next: владелец смотрит; VK ID ждёт ответа devsupport.

## 2026-09-10 · wildcar.org: лента на главной, разделы, стиль обложки; `date` в шапке страниц
- What: сайт перестроен (репозиторий wildcar-site): главная — общая лента новостей и картин дня из 30 карточек, новые сверху, собирается хуком `hooks/feed.py` при каждой сборке; вкладки «Позитивные новости», «Картина дня», «Интересное» (статьи владельца, список тоже строит хук), «О проекте», «Контакты»; палитра обложки ВКонтакте (`posinus.css`), четыре логотипа каналов в подвале, Open Graph на всех страницах (у новостей — первая картинка и первый абзац). Публикатор и daypic пишут в YAML-шапку `date` (ISO 8601) — по нему хук сортирует ленту; у старых страниц берётся mtime. 350 тестов.
- Why: просьба владельца причесать сайт: два текущих раздела, лента как на других площадках в порядке публикации, раздел для своих статей, гамма картинки ВКонтакте, описание «о проекте», контакты четырьмя логотипами.
- Files: pipeline/publisher.py, pipeline/daypic.py, pipeline/tests/test_publisher.py, pipeline/tests/test_daypic.py, pipeline/AGENTS/SPEC.md, pipeline/docs/services.md; wildcar-site: mkdocs.yml, hooks/feed.py, overrides/, docs/index.md, docs/about.md, docs/contacts.md, docs/interesting/, docs/assets/
- Next: владелец смотрит сайт; при желании — карточки и на страницах разделов, постраничный архив.
...
- Files: pipeline/publisher.py, pipeline/daypic.py, pipeline/tests/test_publisher.py, pipeline/AGENTS/SPEC.md, pipeline/docs/services.md, pipeline/deploy/pipeline.env.example, ~/repo/wildcar-site/docs/assets/logos/
- Next: посмотреть первый живой пост слота 15:00 МСК на всех площадках, особенно ряд логотипов на wildcar.org.

## 2026-09-10 · VK: прямые ссылки на картинки первыми строками поста
- What: в режиме `link` пост открывается прямыми ссылками на файлы картинок на wildcar.org (поле `image_urls`, собирается из адреса страницы wildcar.org и имён файлов перед каждой отправкой; у картины дня горизонтальный файл), потом заголовок и остальное. Сухой прогон печатает число ссылок. 4 теста, 347 всего.
- Why: владелец превращает такие ссылки в фото, редактируя пост вручную: веб-клиент под сессией админа скачивает картинку и прикрепляет её, API с ключом сообщества этого не делает (запись 428). PNG в клиенте тоже проходит, отказ по PNG касается только серверов загрузки API.
- Files: pipeline/publisher.py, pipeline/daypic.py, pipeline/tests/{test_publisher,test_daypic}.py, pipeline/AGENTS/SPEC.md, pipeline/docs/services.md
- Next: ID приложения VK ID от владельца, письмо в devsupport.
...
- Files: pipeline/publisher.py, pipeline/daypic.py, pipeline/notify.py, pipeline/tests/{test_publisher,test_daypic}.py, pipeline/AGENTS/SPEC.md, pipeline/docs/services.md, pipeline/deploy/pipeline.env.example, crawler/collector/services/daypic.py
- Next: первая живая история с утренним выпуском 11 сентября (08:00 МСК); сегодняшний выпуск уже закрыт и историю не получит.

## 2026-09-10 · VK: путь через сервер сообщений закрыт, режим photo с ключом сообщества падает с подсказкой
- What: убраны `VK_PHOTO_UPLOAD` и загрузка через `photos.getMessagesUploadServer`; в режиме `photo` ошибка 27 от сервера загрузки стены превращается в ошибку с подсказкой «set VK_POST_MODE=link». Перекодировка PNG в JPEG осталась. Проба сообщает, что загрузка через сервер сообщений для стены бесполезна. 337 тестов. Код уехал коммитом 262bd54, документы следом.
- Why: запись 426 с фото, принадлежащим самому сообществу, вышла без картинки, как и 425: `wall.post` принимает такое вложение и молча выбрасывает (og:image записи заглушка VK, у записей 418/415/405 с настоящими фото там фото). Вложение-ссылка даёт 100 «No photo given» для любого хоста, t.me и Википедия включительно. Картинки вернёт только пользовательский токен с правами wall и photos.
...
- Next: решение владельца — писать ли в devsupport@corp.vk.com за правами wall и photos для Standalone-приложения VK ID; пока VK в режиме `link`, посты текстом с карточкой без картинки.
...
- Files: pipeline/publisher.py, pipeline/daypic.py, pipeline/tools/vk_probe.py, pipeline/tests/{test_publisher,test_daypic}.py, pipeline/AGENTS/SPEC.md, pipeline/docs/services.md, pipeline/deploy/pipeline.env.example
- Next: владелец кладёт ключ сообщества в `VK_ACCESS_TOKEN`, ставит `VK_POST_MODE=link`, удаляет пробные записи 420 и 421 из «Отложенных»; первый живой пост посмотреть глазами: есть ли у карточки картинка.

## 2026-09-10 · ВКонтакте: токен Kate Mobile умер, разбор и проба ключа
- What: разобран отказ «9 Flood control». Токен получает его на любой метод, даже на users.get; хост чист (вызов без токена с того же IP проходит), нагрузка около 50 вызовов в день, сообщество открыто. Причина у VK: с 7 сентября квоты API считаются по приложениям и помесячно, 8 сентября Kate Mobile (app_id нашего токена) отключили. Новый `tools/vk_probe.py` показывает, что умеет ключ (права, загрузка фото, отложенный wall.post со ссылкой, который тут же удаляется). Документы обновлены, код публикации не тронут.
- Why: тревоги `platform:vk` и `daypic-platform:vk` 10 сентября; с 8 сентября 15 новостей и две картины дня вышли без VK, каждая сжигая по 8 попыток.
- Files: pipeline/tools/vk_probe.py, pipeline/docs/services.md, pipeline/AGENTS/STATE.md, AGENTS/MEMORY.md
- Next: решение владельца. Обнулить VK_ACCESS_TOKEN, чтобы прекратить попытки и тревоги; создать ключ сообщества и прогнать пробу (если wall.post проходит, делаем режим поста-ссылки в publisher и daypic); написать в devsupport за правами wall и photos; или отказаться от VK.
- Хвост: владелец создал ключ сообщества и прогнал пробу. У ключа есть право wall (в документации его нет), фото не грузится (27), а `wall.post` со ссылкой на главную wildcar.ru упал на 100 «link_photo_sizing_rule»: у главной нет og:image, у страниц статей есть. Проба теперь шлёт три отложенные записи (текст, ссылка вложением на страницу с картинкой, ссылка в тексте) и удаляет их.

## 2026-09-06 · daypic.py --day: перерисовать выпуск за прошлую дату
- What: ключ `--day ГГГГ-ММ-ДД` делает выпуск за указанный местный день (дата в промпте, заголовке и именах файлов), снимает проверку времени и лимит попыток; опубликованный день не трогается. 3 теста, 327 всего.
- Why: выпуски 5 и 6 сентября сгорели по четыре попытки из-за снятой Codex модели gpt-5.4; владелец починил роутер (gpt-5.5) и попросил нарисовать оба вручную и разослать — заодно живая проверка новой ведущей модели.
- Files: pipeline/daypic.py, pipeline/tests/test_daypic.py, pipeline/AGENTS/SPEC.md, pipeline/docs/services.md
- Next: прогнать на проде `--day 2026-09-05` и `--day 2026-09-06`, посмотреть ориентацию кадров в журнале.
- Хвост: прогнано на проде 20:45–20:52 UTC от сервисного пользователя, оба выпуска на четырёх площадках (t.me/posinus/854, 855; wildcar.org/kartina/2026-09-05, -06; VK 404, 405). gpt-5.5 как ведущая модель ориентацию из промпта читает: все четыре кадра ровно 1024x1536 и 1536x1024, предупреждений нет. Стили ink-wash и chalkboard-art, подписи про настоящие праздники дат, без ссылок.
...
- Хвост: проверено живым вызовом через выкаченный код (журнал роутера, строка 16435). Тем же вечером владелец сменил адрес на `https://wildcar.org` — заменён везде, кроме журналов HISTORY; повторная живая проверка — строка 16439.
...
- Files: код не менялся; операции над `evaluator.sqlite3` и `/var/lib/posinus/wildcar-org`.
- Next: копии в Telegram (t.me/posinus/751), VK (wall-233237778_301) и на wildcar.ru остаются — снимать ли, решает владелец.

## 2026-08-26 · Смена моделей: glm на отборе, terra на картинках, sol на картине дня
- What: оценщик работает на `z-ai/glm-5.3-flash` через openrouter (env правил владелец; первая прод-пачка 8/8 за $0.0029), визуальная проверка картинок по умолчанию `gpt-5.6-terra` (дефолт в коде, живая проба ответила terra), слот картины дня `day` переведён на `gpt-5.6-sol` (строка слота через manage.py shell; build_prompt ответил sol с реальными праздниками 26 августа). Пересказ остаётся на deepseek-v4-pro.
- Why: DeepSeek поднял цены до 4x с 16–17 августа; прогон 100 новостей выбрал glm — лучшее попадание в текущий отбор при ~$3/мес. terra и sol — выбор владельца.
- Files: pipeline/preparer.py, pipeline/docs/services.md, pipeline/deploy/pipeline.env.example; слот картины дня — данные базы краулера.
- Next: владелец применяет профиль r5 через «Отбор» (без него отбор на glm бежит вполсилы); не запускать --rescore-all через смену модели; посмотреть завтрашний выпуск картины дня на sol.

## 2026-08-26 · Финальный контроль уместности перед вердиктом
- What: новость, прошедшую пороговый профиль, оценщик показывает модели второй раз — как выпускающему редактору ленты позитивных новостей. Вердикт «неуместно» бракует её: `decision='not_positive'`, в reason ответ модели с пометкой «Финальный контроль», в `selector_version` суффикс `+veto`, который `--rescore-all` не переворачивает. Сбой проверки оставляет новость в очереди; выключатель `EVALUATOR_FINAL_CHECK=off`. 313 тестов.
- Why: некролог Долли Партон (news 12306) прошёл профиль r4 честно — позитивность 8, вдохновение 9, а негативность профиль не ограничивает вовсе — и 2026-08-26 10:01 UTC ушёл на все четыре площадки с заголовком «Умерла Долли Партон…». Проверено живьём через роутер: 12306 бракуется («главное событие — смерть»), история преодоления 11471 (наводнение, потом золото по карате) проходит.
- Files: pipeline/evaluator.py, pipeline/tests/test_evaluator.py, pipeline/AGENTS/SPEC.md
- Next: снятие публикации 12306 — решение владельца; ревизия модели и цен (deepseek-v4-pro с 2026-08-18 в пересчёте на токен дороже в два с половиной — три раза).

## 2026-08-16 · Конвейер встал без сырья: планка вернулась к 8, подготовка упёрлась в лимит токенов
- What: со стороны конвейера кода не менялось — разбор простоя и две операции на проде. Профиль отбора переведён в редакцию 4 (`highlight_min` 8 по всем семи осям, решение владельца), `evaluator.py --backfill --rescore-all` исправил 174 вердикта под r4. Из них 164 новости уже вычищены ретеншеном, поэтому в очередь подготовки встали 8 живых, свежих (14–15 августа): подготовщик сделал одну, публикатор впервые за двое суток пишет `prepared 1`, ближайший слот 09:00 МСК.
- Why: публикатор двое суток писал `prepared 0`, подготовщик `queue 0`, оценщик `0 selected`. Канал с 26 июля выпускал 178 новостей против 100 отобранных и жил накопленным запасом, который кончился 14 августа; редакция 3 от 11 августа уронила долю отбора с 4.51% до 1.60%. Подробности со стороны краулера — в его HISTORY за ту же дату.
- Files: код не менялся; операции над `/var/lib/posinus/posinus.sqlite3` (профиль) и прогон `posinus-evaluator-backfill.service`.
- Открылось: четыре подготовки из пяти упали на «DeepSeek stopped at max_tokens=4000 before returning any content» — `PREPARER_MAX_TOKENS` мал для нынешнего промпта пересказа. Правка `/etc/posinus/pipeline.env` отклонена политикой доступа агента, остаётся владельцу; строки со статусом `error` подготовщик берёт заново сам.
- Next: поднять `PREPARER_MAX_TOKENS` до 8000 и прогнать подготовщик; посмотреть, хватает ли отбора на 8 слотов в день.

## 2026-08-11 · Новость 8949 снята с wildcar.org решением владельца
- What: страница `/news/8949/` удалена, индекс и RSS перегенерированы штатными функциями публикатора уже без неё, строка `publication` переведена в `status='removed'` с причиной (строки не удаляются — SPEC, «Идемпотентность и метки»). Проверено: страница отвечает 404, в индексе и ленте упоминаний нет.
- Why: боевые FPV-дроны для армии США — не новость для канала позитива; источник shotam.info заблокирован на стороне краулера (его HISTORY от той же даты).
- Files: код не менялся; операция над `/var/lib/posinus/wildcar-org` и `evaluator.sqlite3`; pipeline/AGENTS/SPEC.md описывает статус `removed`.
- Next: посты той же новости в Telegram (t.me/posinus/650), VK (wall-233237778_199) и на wildcar.ru остались — снимать ли их, решает владелец.

## 2026-08-11 · Заголовок рассказывает новость сам, посты выходят с призывом
- What: промпт пересказа требует заголовок, понятный без открытия текста (кто и что сделал; тизеры вроде «Незнакомка с подносом» запрещены с образцом замены). Публикатор дописывает каждой новости постоянную концовку владельца — призыв подписаться плюс вопрос читателю — со словом под площадку: «на канал» в телеграме, «на сообщество» в VK, ссылка на t.me/posinus на wildcar.ru, wildcar.org и в ленте Дзена. 297 тестов. LIVE at `73989ec`, проверено сухим прогоном новости 8949 по всем четырём площадкам на проде.
- Why: отзыв владельца по каналу: тизерный заголовок заставляет открывать статью, а посту не хватало человеческого обращения. Концовка — правило публикации, как постоянные теги: не хранится в пересказе и достаётся уже подготовленным новостям; в телеграме она входит в лимит видимого текста и не выбрасывается — уступают абзацы.
- Files: pipeline/preparer.py, pipeline/publisher.py, pipeline/tests/test_publisher.py, pipeline/AGENTS/SPEC.md
- Next: посмотреть заголовки первых пересказов на новом промпте.

## 2026-08-11 · Описание картины дня без ссылок на источники
- What: формат ответа чат-модели требует описание без ссылок и списка источников, а `strip_source_links()` вычищает утёкшее (markdown-ссылки, голые адреса, хвост «Источники: …») перед сохранением. 292 теста. LIVE at `0bf793c`.
- Why: с включённым веб-поиском gpt-5.5 дописывала в подпись хвост «Источники: calend.ru/…» — выпуски 10 и 11 августа ушли с ним. Подпись под картинкой — текст для читателя, а не библиография.
- Files: pipeline/daypic.py, pipeline/tests/test_daypic.py, pipeline/AGENTS/SPEC.md
- Verified: живьём через роутер — `build_prompt` за 11 августа вернул чистое описание без единого адреса; зачистка на реальной подписи выпуска 2026-08-11 снимает ровно хвост источников. Вышедшие посты 10–11 августа не правились.

## 2026-08-09 · Картину дня забирают снаружи: манифест вместо гадания по каталогу
- What: рядом с картинками пишется `<дата>-<слот>.json` (день, слот, заголовок, описание, точные имена вертикального и горизонтального файлов, стиль, модель, время) — последним и через `os.replace`, до площадок; `retention.py` сносит его вместе с картинками. 289 тестов. LIVE at `d6915d6`.
- Why: владелец решил, что бот `@wildaiapi_bot` сам заберёт вертикальную картинку в 8:00 и разошлёт её подписчикам. Каталога для этого мало: расширение не закреплено (тяжёлый PNG пережимается в JPEG, исходник удаляется), читатель мог поймать недописанный файл, а описание дня жило только в своей базе конвейера, куда чужим нельзя.
- Files: pipeline/{daypic.py,retention.py}, pipeline/tests/test_daypic.py, pipeline/{AGENTS/SPEC.md,docs/services.md,README.md}, ../AGENTS/{ENV.md,MEMORY.md}
- Verified: на проде — 11 манифестов дописаны задним числом для лежащих файлов, сегодняшний отдаёт настоящее описание 9 августа и `2026-08-09-day.jpg`; `keeper` без sudo читает каталог по узкому ACL (проход по двум родителям, чтение в `daypic/`, default-запись для новых файлов проверена пробником), база краулера ему по-прежнему закрыта; прогон сервиса на новом коде чистый.
- Next: сторона бота — читать манифест в `~/repo/ort_bot` и выключить там свою генерацию картинки дня (владелец сказал, что распорядится).
