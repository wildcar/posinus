...
Страница «Новости» показывает до 200 логических новостей. Список сортируется по дате или по имени источника в обе стороны и фильтруется по источнику и по решению отборщика.

Отдельная панель «Пороги оценок» выводит сразу все 20 характеристик. У каждой шкалы от 0 до 10 два ползунка: нижний и верхний порог. Суженный диапазон оставляет только новости, у которых последняя оценка по этой характеристике попадает в интервал, поэтому новости без оценок при любом суженном диапазоне отсеиваются.
